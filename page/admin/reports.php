...
$query = $conn->query("SELECT * FROM feedbacks");
  ...
  <div id="main-content" class="flex-1 flex-col min-h-screen transition-all duration-300 ease-in-out w-full">
   ...
   <header class="md:hidden w-full bg-card border-b border-border p-4 flex justify-between items-center sticky top-0 z-30">
      <div class="flex items-center gap-3">
        <h2 class="text-base sm:text-lg md:text-xl font-semibold truncate">Reports</h2>
      </div>
...
<?php
  ...
  while($row = $query->fetch_assoc()){
    ...
    $userQuery = $conn->query("SELECT * FROM users WHERE email = '". $userEmail ."' LIMIT 1");
    $user = $userQuery->fetch_assoc();

    //get profile of image using email
